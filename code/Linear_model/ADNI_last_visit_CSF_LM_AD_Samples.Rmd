...
pheno.ad <- pheno.last.visit %>% filter(DX == "Dementia") %>% as.data.frame()
rownames(pheno.ad) <- pheno.ad$barcodes
beta.ad <- beta.all[,match(rownames(pheno.ad), colnames(beta.all))]
```
...
ABETA.results.ad.lme <- plyr::adply(
  ...
  .margins = 1,
  .fun = function(one){
    dat.ad <- data.frame (beta = one, pheno.ad)
    tryCatch({
      suppressMessages({
        b <- lm(
...
write_csv(
  ...
  file.path(dir.results.raw, "ADNI_Last_Visit_CSF_ABETA_with_covariates_LM_AD_results.csv")
)
### TAU
doParallel::registerDoParallel(10)
...
write_csv(
  TAU.results.ad.lme,
  file.path(dir.results.raw, "ADNI_Last_Visit_CSF_TAU_with_covariates_LM_AD_results.csv")
)
### PTAU
doParallel::registerDoParallel(10)
...
write_csv(
  PTAU.results.ad.lme,
  file.path(dir.results.raw, "ADNI_Last_Visit_CSF_PTAU_with_covariates_LM_AD_results.csv")
)
```
...
writexl::write_xlsx(
  ...
  file.path(dir.results.inflation , "ADNI_Last_Visit_AD_inflation_and_bias_table.xlsx")
)
```
